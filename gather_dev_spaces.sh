...
for ns in "${UNIQUE_NAMESPACES[@]}"; do
  if [ -n "$ns" ]; then
    ...
    # Events
    oc get events -n "$ns" -o yaml \
      > "$LOGS_DIR/operator-namespaces/${ns}/core-resources/events.yaml" 2>/dev/null || true
...
oc adm inspect packagemanifests --dest-dir="$LOGS_DIR" || true
...
if [ "${#WS_NAMESPACES[@]}" -gt 0 ]; then
  ...
  for ws_ns in "${WS_NAMESPACES[@]}"; do
    ...
    if [ -n "$ns_name" ]; then
      ...
      # Events
      oc get events -n "$ns_name" -o yaml \
        > "$LOGS_DIR/workspace-namespaces/${ns_name}/core-resources/events.yaml" 2>/dev/null || true
...
oc adm inspect event -A --dest-dir="$LOGS_DIR" || true
